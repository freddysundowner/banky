...
Selection logic (checked in order):
  ...
  TENANT_BACKEND=local  → always use local Postgres (requires LOCAL_PG_SUPERUSER_URL)
  ...
  TENANT_BACKEND not set → auto-detect:
      ...
      2. LOCAL_PG_SUPERUSER_URL present → local
      3. Neither                    → shared (master DB fallback)
"""
...
def get_tenant_backend() -> str:
    ...
    if os.environ.get("NEON_API_KEY"):
        return "neon"
    if os.environ.get("LOCAL_PG_SUPERUSER_URL"):
        return "local"
    return "shared"
