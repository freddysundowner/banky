...
def contact_to_dict(c: CrmContact) -> dict:
    return {
        ...
        "assigned_to_name": f"{c.assigned_to.first_name} {c.assigned_to.last_name}" if c.assigned_to else None,
...
def interaction_to_dict(i: CrmInteraction) -> dict:
    return {
        "id": i.id,
        ...
        "created_by_name": f"{i.created_by.first_name} {i.created_by.last_name}" if i.created_by else None,
        "created_at": i.created_at.isoformat() if i.created_at else None,
    }

def followup_to_dict(f: CrmFollowUp, contact: CrmContact = None) -> dict:
    now = datetime.utcnow()
    is_overdue = f.status == "pending" and f.due_date < now
    return {
        "id": f.id,
        ...
        "assigned_to_name": f"{f.assigned_to.first_name} {f.assigned_to.last_name}" if f.assigned_to else None,
...
def list_contacts(
    ...
    tenant_session, membership = get_tenant_session_context(organization_id, current_user, db)
...
@router.post("/{organization_id}/crm/contacts")
def create_contact(
    organization_id: str,
    body: ContactCreate,
    db: Session = Depends(get_db),
    ...
    require_not_demo(organization_id, db)
    tenant_session, membership = get_tenant_session_context(organization_id, current_user, db)
...
@router.put("/{organization_id}/crm/contacts/{contact_id}")
def update_contact(
    organization_id: str,
    contact_id: str,
    body: ContactUpdate,
...
@router.delete("/{organization_id}/crm/contacts/{contact_id}")
def delete_contact(
    organization_id: str,
    contact_id: str,
    db: Session = Depends(get_db),
...
@router.post("/{organization_id}/crm/contacts/{contact_id}/convert")
def convert_contact(
    organization_id: str,
    contact_id: str,
    body: ConvertRequest,
...
def list_all_interactions(
    ...
    interactions = tenant_session.query(CrmInteraction).order_by(desc(CrmInteraction.interaction_date)).limit(200).all()
    result = []
    for i in interactions:
        d = interaction_to_dict(i)
        if i.contact:
            d["contact_name"] = f"{i.contact.first_name} {i.contact.last_name}"
        result.append(d)
    return result


@router.post("/{organization_id}/crm/contacts/{contact_id}/interactions")
...
@router.delete("/{organization_id}/crm/interactions/{interaction_id}")
def delete_interaction(
    ...
    interaction = tenant_session.query(CrmInteraction).filter(CrmInteraction.id == interaction_id).first()
    if not interaction:
...
def list_followups(
    ...
    if status == "overdue":
        q = q.filter(CrmFollowUp.status == "pending", CrmFollowUp.due_date < datetime.utcnow())
    elif status:
    ...
    result = []
    for f in followups:
        contact = tenant_session.query(CrmContact).filter(CrmContact.id == f.contact_id).first()
        result.append(followup_to_dict(f, contact))
    return result


@router.post("/{organization_id}/crm/followups")
def create_followup(
    organization_id: str,
    body: FollowUpCreate,
    db: Session = Depends(get_db),
    ...
    contact = tenant_session.query(CrmContact).filter(CrmContact.id == body.contact_id).first()
    if not contact:
        raise HTTPException(status_code=404, detail="Contact not found")
...
@router.put("/{organization_id}/crm/followups/{followup_id}")
def update_followup(
    organization_id: str,
    followup_id: str,
    body: FollowUpUpdate,
    ...
    followup = tenant_session.query(CrmFollowUp).filter(CrmFollowUp.id == followup_id).first()
    if not followup:
    ...
    tenant_session.commit()
    tenant_session.refresh(followup)
    contact = tenant_session.query(CrmContact).filter(CrmContact.id == followup.contact_id).first()
    return followup_to_dict(followup, contact)


@router.delete("/{organization_id}/crm/followups/{followup_id}")
def delete_followup(
    organization_id: str,
    followup_id: str,
    db: Session = Depends(get_db),
...
def get_crm_stats(
    ...
    for status in ["new", "contacted", "qualified", "converted", "lost"]:
        by_status[status] = tenant_session.query(CrmContact).filter(CrmContact.status == status).count()
    overdue = tenant_session.query(CrmFollowUp).filter(
        CrmFollowUp.status == "pending",
        CrmFollowUp.due_date < now
    ).count()
    pending_followups = tenant_session.query(CrmFollowUp).filter(
        CrmFollowUp.status == "pending"
    ).count()
