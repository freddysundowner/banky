      ...
      <div className="bg-white rounded-2xl p-8 border border-gray-200">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <ul className="space-y-3 text-gray-600">
              <li className="flex items-start gap-3"><ArrowRight className="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5" /><div><strong className="text-gray-900">Node.js 18+</strong> (v20 LTS recommended) — install manually before running the installer</div></li>
              <li className="flex items-start gap-3"><ArrowRight className="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5" /><div><strong className="text-gray-900">Python 3.11+</strong> — auto-installed by the installer via pyenv if your system version is too old</div></li>
              <li className="flex items-start gap-3"><ArrowRight className="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5" /><div><strong className="text-gray-900">PostgreSQL 14+</strong> — auto-detected and configured by the installer; or use a managed database (AWS RDS, Neon, Supabase)</div></li>
              <li className="flex items-start gap-3"><ArrowRight className="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5" /><div><strong className="text-gray-900">Nginx</strong> (reverse proxy, for production deployments)</div></li>
            </ul>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-8 border border-gray-200">
        ...
        <Tip type="success">
          <strong>Fully automated setup.</strong> The installer handles Python, PostgreSQL, dependencies, database creation, migrations, and the frontend build — all in one command. Node.js 18+ is the only thing you need to install manually beforehand.
        </Tip>
...
cd banky`}</CodeBlock>
          ...
          <div className="border-l-4 border-purple-500 pl-6">
            ...
            <p className="text-gray-600 mb-4">One command sets everything up:</p>
            <CodeBlock>{`chmod +x install.sh
./install.sh`}</CodeBlock>
            <p className="text-gray-600 mt-6 mb-3">The installer runs 6 steps automatically:</p>
            <ul className="space-y-2 text-sm text-gray-600 ml-4">
              <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" /><span><strong>Prerequisites:</strong> Verifies Node.js; auto-installs Python 3.11+ via pyenv if your system version is too old</span></li>
          ...
          <div className="border-l-4 border-purple-500 pl-6">
            <div className="flex items-center gap-3 mb-3">
              <StepNumber n={3} />
              <h3 className="font-semibold text-gray-900 text-lg">Configure Environment (optional)</h3>
            </div>
            <p className="text-gray-600 mb-4">
              The installer writes a working <code className="bg-gray-100 px-1.5 py-0.5 rounded text-sm">.env</code> automatically. You only need to edit it if you want to change the port or other defaults:
            </p>
...
# Session encryption secret (auto-generated by the installer)
SESSION_SECRET=change-this-to-a-random-string-at-least-32-characters

# License key (optional — all features are unlocked by default)
# LICENSE_KEY=BANKY-ENT-PERP-XXXXXXXX`}</CodeBlock>
            <Tip type="info">
              <strong>Integrations are configured in the app, not in .env.</strong> SMS provider keys, email (Brevo) API keys, M-Pesa credentials, and payment gateway keys are all set through the <strong>Settings</strong> page inside BANKY after you log in — not via environment variables.
            </Tip>
          </div>
          ...
          <div className="border-l-4 border-purple-500 pl-6">
            ...
            <p className="text-gray-600 mb-3"><strong>For production</strong> (recommended) — use PM2:</p>
            <CodeBlock>{`pm2 start ecosystem.config.js
pm2 save
pm2 startup`}</CodeBlock>
            <p className="text-gray-600 mt-5 mb-3"><strong>For a quick test</strong> — use the start script:</p>
            <CodeBlock>{`./start.sh

# App:  http://localhost:5000
# API:  http://localhost:8000`}</CodeBlock>
            <Tip type="info">
              <strong>start.sh</strong> automatically finds free ports if 5000 or 8000 are already in use — useful when testing alongside other services. See the <strong>PM2 Process Manager</strong> section for the full production setup including auto-start on reboot.
            </Tip>
          </div>
