...
export default function Testimonials() {
  ...
  useEffect(() => {
    fetch('/api/public/landing-content/testimonials')
      .then(res => res.json())
      .then(data => { if (data.data) setTestimonials(data.data); })
      .catch(() => {});
  }, []);
    ...
    <section id="testimonials" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
                ...
                >
                  <div className="relative bg-gray-50 rounded-xl p-6 border border-gray-100 h-full">
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full ${bgColors[index % bgColors.length]} text-white flex items-center justify-center text-sm font-semibold`}>
                        {getInitials(testimonial.name)}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900 text-sm">{testimonial.name}</p>
                        <p className="text-gray-500 text-xs">{testimonial.role}, {testimonial.organization}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 w-10 h-10 bg-white rounded-full shadow-md border border-gray-200 flex items-center justify-center text-gray-600 hover:text-gray-900 transition">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 w-10 h-10 bg-white rounded-full shadow-md border border-gray-200 flex items-center justify-center text-gray-600 hover:text-gray-900 transition">
        ...
        <div className="flex justify-center gap-2 mt-8">
          {Array.from({ length: maxIndex + 1 }).map((_, i) => (
            <button key={i} onClick={() => setCurrentIndex(i)} className={`w-2.5 h-2.5 rounded-full transition ${i === currentIndex ? 'bg-blue-600' : 'bg-gray-300'}`} />
          ))}
        </div>
      </div>
    </section>
