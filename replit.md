...
- **CRM**: Contact management with interaction logging, follow-up scheduling, contact-to-member conversion, and pipeline stats.
- **Soft Loan System**: Member-facing instant micro-loans with configurable scoring (F1–F7 formulas: savings threshold, shares threshold, no arrears, repayment history, consistent deposits, long-term member, transaction activity) plus hard gates (active account, no existing soft loan, minimum membership duration). Fixed 1-month term, auto-approved. Admin configurable via Settings. Mobile endpoints at `/api/mobile/me/soft-loan/*`. `is_soft_loan` column on `loan_applications` + `soft_loan_configs` table added via migration v35.
- **Collateral & Insurance**: Full collateral lifecycle management (register, valuate, lien, release, liquidate); insurance policy tracking with expiry alerts; 9 auto-seeded collateral types with configurable LTV %; overdue revaluation and expiring insurance alerts dashboard. **Valuers Registry** (CRUD management of approved valuer firms/individuals, linked to valuation records). **Valuation document upload** (PDF/image/Word report attached per item, served at `/uploads/valuations/`). DB migration v31 adds `valuers` table + `valuer_id`/`valuation_document_path` columns on `collateral_items`. Frontend at `client/src/components/collateral.tsx`, backend at `python_backend/routes/collateral.py`.
...
- **Business Model**: Institution-type-based subscription plans (Chama, SACCO, MFI, Bank) with Small/Large tiers for both SaaS (monthly subscription) and Enterprise (perpetual licence). 16 total plans. Generic plans removed. Registration requires institution type selection which determines the trial plan. Database-driven feature flags and a unified license key system. Configurable trial period.
